...
r = 0.025
a = 0.25
prob = ODEProblem(NetworkExFKPP, p0, (0.0,40.0), [r,a])
...
begin
    ...
    ticksize=18, ticks=collect(0.0:0.5:3.5), ticklabelsize=15, labelpadding=3)
    Label(g1[1:2, 0], "SUVR", fontsize=35, rotation = pi/2)
    
    ax = Axis(g2[1:2,1:4],
            xautolimitmargin = (0, 0), xgridcolor = (:grey, 0.05), xgridwidth = 2,
            xticklabelsize = 25, xticks = 1:1.5:3, xticksize=18,
    ...
    hidespines!(ax, :t, :r)
    for i in 1:36
        lines!(allsol.t, allsol[i, :], linewidth=1.5, color=(braak_colors[1], 0.5))
    end
    ax = Axis(g2[3:4,1:4],
    xautolimitmargin = (0, 0), xgridcolor = (:grey, 0.05), xgridwidth = 2,
    ...
    yautolimitmargin = (0, 0), ygridcolor = (:grey, 0.5), ygridwidth = 2,
    yticklabelsize = 25, yticks = collect(0:0.5:1.), yticksize=18
    )
    ...
    Legend(g2[5, 1:4],
        ...
        patchsize = (50, 30), colgap = 10, orientation = :horizontal, fontsize=30)
