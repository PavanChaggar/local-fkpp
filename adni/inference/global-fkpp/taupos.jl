...
pst = sample(m,
             ...
             n_chains,
             progress=true)
serialize(projectdir("adni/new-chains/global-fkpp//length-free/pst-taupos-$(n_chains)x$(n_samples).jls"), pst)

# calc log likelihood 
log_likelihood = pointwise_loglikelihoods(m, MCMCChains.get_sections(pst, :parameters));
serialize(projectdir("adni/new-chains/global-fkpp/length-free/ll-taupos-$(n_chains)x$(n_samples).jls"), log_likelihood)