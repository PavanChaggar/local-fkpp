...
local_pst = deserialize(projectdir("adni/chains/local-fkpp/pst-taupos-2000-indp0.jls"));
local_pst["u[1,1]"] |> vec |> scatter
local_pst[:Pm] |> vec |> hist
local_pst[:Am] |> vec |> hist
local_meanpst = mean(local_pst)
local_params = [[local_meanpst[Symbol("ρ[$i]"), :mean], local_meanpst[Symbol("α[$i]"), :mean]] for i in 1:27];
local_inits = [[local_meanpst["u[$i,$j]", :mean] for i in 1:72] for j in 1:27]
local_sols = [solve(
                ODEProblem(NetworkLocalFKPP, init, (0.0,5.0), p), Tsit5(), saveat=t) 
                for (init, t, p) in zip(local_inits, times, local_params)];
#-------------------------------------------------------------------------------
# Global FKPP
#-------------------------------------------------------------------------------
...
function elppd_local_fkpp(pst, subdata, initial_conditions, times)
    ps, as, σ = vec(pst[:Pm]), vec(pst[:Am]), vec(pst[:σ])
    sols = Vector{Vector{Float64}}()
    for (p, a) in zip(ps, as)
        _prob = ODEProblem(NetworkLocalFKPP, initial_conditions, (0.,5.), [p, a])
        _sol = solve(_prob, Tsit5(), saveat=times)
        push!(sols, _sol[2])
    end

    lls = Array{Float64}(undef, 72, 8000)
    for r in 1:72
        for s in 1:8000
            lls[r, s] = loglikelihood(Normal(sols[s][r], σ[s]), subdata[1, 2])
        end
    end
    sum(sum(lls, dims=2) ./ 8000)
end

elppd(pst, subdata[1], initial_conditions[1], times[1])

#-------------------------------------------------------------------------------
# Model comparison
#-------------------------------------------------------------------------------
function get_maximum_pst(pst)
    maximum(pst[:lp])
end

function calc_aic(pst)
    k = length(pst.name_map.parameters)
    maxP = get_maximum_pst(pst)
...
get_maximum_pst(local_pst)
get_maximum_pst(logistic_pst)
get_maximum_pst(global_pst)
get_maximum_pst(diffusion_pst)
...
findall(x -> x > 1.0, local_loo.psis_object.pareto_k)
