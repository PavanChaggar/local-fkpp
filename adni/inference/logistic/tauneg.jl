...
pst = sample(m, 
             ...
             n_chains,
             progress=true)
serialize(projectdir("adni/chains/logistic/pst-tauneg-$(n_chains)x$(n_samples).jls"), pst)

# calc log likelihood 
pst = deserialize(projectdir("adni/chains/logistic/pst-tauneg-$(n_chains)x$(n_samples).jls"));
log_likelihood = pointwise_loglikelihoods(m, MCMCChains.get_sections(pst, :parameters));
serialize(projectdir("adni/chains/logistic/ll-tauneg-$(n_chains)x$(n_samples).jls"), log_likelihood)