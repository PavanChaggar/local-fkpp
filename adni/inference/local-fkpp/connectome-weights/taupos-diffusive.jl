...
pst = sample(m, 
             ...
             n_chains,
             progress=false)
serialize(projectdir("adni/chains/local-fkpp/connectome-weights/pst-taupos-$(n_chains)x$(n_samples)-diffusive.jls"), pst)

# calc log likelihood 
log_likelihood = pointwise_loglikelihoods(m, MCMCChains.get_sections(pst, :parameters));
serialize(projectdir("adni/chains/local-fkpp/connectome-weights/ll-taupos-$(n_chains)x$(n_samples)-diffusive.jls"), log_likelihood)