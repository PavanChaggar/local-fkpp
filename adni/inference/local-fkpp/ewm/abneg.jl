...
# Data 
#-------------------------------------------------------------------------------
data = ADNIDataset(negdf, dktnames; min_scans=3, reference_region="ERODED_SUBCORTICALWM", qc=true)
...
pst = sample(m, 
             ...
             n_samples, 
             n_chains)
serialize(projectdir("adni/chains-revisions/local-fkpp/wm/pst-abneg-$(n_chains)x$(n_samples).jls"), pst)
