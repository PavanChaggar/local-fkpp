...
pst = sample(m,
             ...
             n_samples, 
             n_chains)
serialize(projectdir("adni/new-chains/local-fkpp/length-free/pst-taupos-$(n_chains)x$(n_samples)-lognormal.jls"), pst)

#calc log likelihood 
log_likelihood = pointwise_loglikelihoods(m, MCMCChains.get_sections(pst, :parameters));
serialize(projectdir("adni/new-chains/local-fkpp/length-free/ll-taupos-$(n_chains)x$(n_samples)-lognormal.jls"), log_likelihood)