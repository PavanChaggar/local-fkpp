...
pst = sample(m, 
             ...
             n_samples, 
             n_chains)
serialize(projectdir("adni/new-chains/local-fkpp/length-free/pst-tauneg-$(n_chains)x$(n_samples)-normal.jls"), pst)

#calc log likelihood 
log_likelihood = pointwise_loglikelihoods(m, MCMCChains.get_sections(pst, :parameters));
serialize(projectdir("adni/new-chains/local-fkpp/length-free/ll-tauneg-$(n_chains)x$(n_samples)-normal.jls"), log_likelihood)