...
ensemble_prob = EnsembleProblem(prob, prob_func=make_prob_func(initial_conditions, ones(n_nz), ones(n_nz), times), output_func=output_func)
...
m = localfkpp(vecsubdata, prob, times, u0, cc, n_nz);
m();
...
pst = sample(m, 
             ...
             1_000, 
             progress=true)
serialize(projectdir("adni/chains/local-fkpp/pst-abneg-1000-indp0.jls"), pst)